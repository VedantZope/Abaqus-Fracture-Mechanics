...
param_constraints = [
    ...
    (0.1, 10),    # c2 range: 1e-15 to 1e-13
...
def generate_default_curve_points():
    csv_file_path = r'Flowcurve_RT.csv'
    expt_stress = []
    expt_strain = []
    # Read the CSV file
...
def generate_new_curve_points(new_chromosome):
    ...
    for i in strain_list:
        stress_list.append(generate_y(new_chromosome[0], new_chromosome[1], new_chromosome[2], i))
...
def tournament(fathers):
    ...
    for candidate in participants:
        ...
        for point in range(len(default_x)):
            error.append((default_y[point] - candidate_y[point])**2)
        aptitude_tag = np.sqrt(np.mean(error))
